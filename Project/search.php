		...
		if($_SERVER['REQUEST_METHOD'] == "POST")
		{
			...
			if(empty($search))
			{
				...
				$query = mysql_query("SELECT s_id, ct1, ct2, ct3, ct4 FROM project as P 
										WHERE t_id = '$teacher_id' && c_id = '$course_id' && Section='$section' ORDER BY Series desc, s_id;");
			...
			else
			{
				...
				$query = mysql_query("SELECT s_id, ct1, ct2, ct3, ct4 FROM project as P 
										WHERE t_id = '$teacher_id' && c_id = '$course_id' && Section='$section' && s_id LIKE '%$search%' ORDER BY Series desc, s_id;");
